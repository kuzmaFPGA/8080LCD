module mrb3973_test (
    ...
    reg [16:0] init_rom_data;// = init_rom[init_rom_addr];

    // Розшифровка
    reg is_data;// = init_rom_data[16]; // 1 - команда, 0 - дані
    reg [15:0] cmd_data;// = init_rom_data[15:0];

    // Станова машина
    reg [3:0] state;
    reg [2:0] wr_substate; // Підстан для логіки запису
 ...
 //   reg wr_start;
    reg wr_busy;
//reg wr_trigger; // Новий сигнал для ініціації запису з головного блоку
        ...
        case (state)
            0: begin
                if (pll_locked) begin
                    LCD_RESET_reg <= 0;
                    init_rom_addr <= 0; 
                    delay_counter <= 2 * 8000; // Затримка на 120 мс при 8 МГц
                    state <= 7;
                end
            end
            7: begin
                if (delay_counter > 0) begin
                    ...
                    LCD_RESET_reg <= 1; // Відпускання ресету
                    state <= 2;
                end
            end
            2: begin
                case (wr_substate)
                    0: begin
                        if (!wr_busy && init_rom_addr <= 637) begin
                            wr_busy       <= 1;
                            init_rom_data <= init_rom[init_rom_addr];
                            is_data <= init_rom_data[16]; // 1 - команда, 0 - дані
                            cmd_data <= init_rom_data[15:0];
                            LCD_DATA_reg  <= cmd_data;
                            LCD_RS_reg    <= is_data ? 1 : 0;
                            LCD_CS_reg    <= 0;
                            LCD_WR_reg    <= 0;
                            wr_substate   <= 1;
                        end else if (init_rom_addr > 637) begin
                            delay_counter <= 120 * 8000; // Затримка після ініціалізації
                            state <= 0;
                        end
                    end
                    1: begin
            ...
            3: begin
                case (wr_substate)
                    0: begin
                        if (!wr_busy) begin
                            wr_busy       <= 1;
                            //wr_data       <= 16'h2900; // Вмикання дисплея
                            //wr_is_cmd     <= 1;
                            LCD_DATA_reg  <= 16'h2900;
                            LCD_RS_reg    <= 0;
                            LCD_CS_reg    <= 0;
                            LCD_WR_reg    <= 0;
                            led_1_reg     <= 1;
            ...
            4: begin
                case (wr_substate)
                    0: begin
                        if (!wr_busy) begin
                            wr_busy       <= 1;
                           // wr_data       <= 16'h2c00; // Встановлення адреси
//wr_is_cmd     <= 1;
                            LCD_DATA_reg  <= 16'h2c00;
                            LCD_RS_reg    <= 0;
                            LCD_CS_reg    <= 0;
                            LCD_WR_reg    <= 0;
                            wr_substate   <= 1;
            ...
            5: begin
                case (wr_substate)
                    0: begin
                        if (!wr_busy && pixel_cnt < TOTAL_PIXELS) begin
                            wr_busy       <= 1;
//wr_data       <= RED;
//wr_is_cmd     <= 0;
                            LCD_DATA_reg  <= RED;
                            ...
                            LCD_WR_reg    <= 0;
                            wr_substate   <= 1;
                        end else if (pixel_cnt >= TOTAL_PIXELS) begin
                            state <= 6;
                        end
                    end
                    1: begin
